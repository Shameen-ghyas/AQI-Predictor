...
def fetch_recent_data(sequence_length, max_windows):
    import hopsworks, os
    from datetime import datetime, timedelta

    project = hopsworks.login(api_key_value=os.getenv("HOPSWORKS_API_KEY"))
    fs = project.get_feature_store()
    fg = fs.get_feature_group("aqi_daily_features", version=5)

    hours_needed = sequence_length + max_windows + 10  # small buffer
    cutoff = datetime.utcnow() - timedelta(hours=hours_needed)

    query = fg.filter(fg.time >= cutoff)
    df = query.read()
...
def get_or_create_backtest_fg(project):
    fs = project.get_feature_store()
    return fs.get_or_create_feature_group(
        name="aqi_backtest_results",
        version=1,
        primary_key=["time"],
        event_time="time",
        description="Rolling backtest: actual vs predicted AQI for day1/2/3",
    )
...
def run_backtest(args):
    ...
    df = fetch_recent_data(args.sequence_length, args.max_windows)
